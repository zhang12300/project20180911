...
void ADC1_Init(void)
{
  SIM_SCGC2 |= (SIM_SCGC2_HSADC1_MASK  );
  SIM_SOPT7 &= ~(SIM_SOPT7_HSADC1BALTTRGEN_MASK  | SIM_SOPT7_HSADC1BALTTRGEN_MASK) ;
  SIM_SOPT7 |= SIM_SOPT7_HSADC1BTRGSEL(0);
}
...
void ADC1_Start(HSADCn_Ch_e adc_ch,HSADC_nbit bit)
{
    int ch=0;
    ...
    {
      ch=adc_ch-8;
      ADC1_CFG2 |=ADC_CFG2_MUXSEL_MASK;           // 1选择b  1 ADxxb channels are selected.
    }     
    else
    {
      ch=adc_ch;
      ADC1_CFG2 &=~ADC_CFG2_MUXSEL_MASK;          // 0选择a  0 ADxxa channels are selected.
    }
    //写入 SC1A 启动转换
...
{
  int result = 0;
  ADC1_Start(HSADC1B_CH11,HSADC_8bit);      //启动ADC转换
  while ((ADC1_SC1(0) & ADC_SC1_COCO_MASK )!= ADC_SC1_COCO_MASK);   //只支持 AB通道
  result = ADC1_R(0);
